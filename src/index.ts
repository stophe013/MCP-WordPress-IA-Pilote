...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        // On applique une troncation pour éviter de faire planter l'IA avec trop de logs
        return truncateContent(response.data);
...
function truncateContent(data: any, maxLength: number = 25000): any {
    if (!data) return data;

    // Si c'est un objet MCP standard avec content array
        ...
        data.content = data.content.map((item: any) => {
            if (item.type === 'text' && typeof item.text === 'string') {
                if (item.text.length > maxLength) {
                    return {
                        ...item,
                        text: item.text.substring(0, maxLength) + `\n\n[... TRUNCATED (${item.text.length - maxLength} chars removed) ...]`
                    };
                }
            }
            return item;
    ...
    // Si c'est directement une string (cas rare mais possible)
